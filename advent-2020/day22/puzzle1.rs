// puzzle1.rs
//
// Advent of Code Day 22 Puzzle 1.
...
fn play_recursive_combat(mut d1: VecDeque<u64>, mut d2: VecDeque<u64>) -> (PlayerId, u64) {
    let mut memo1: HashSet<String> = HashSet::new();
    let mut memo2: HashSet<String> = HashSet::new();

    while !d1.is_empty() && !d2.is_empty() {
        let encoded1 = encode_round(&d1);
        let encoded2 = encode_round(&d2);
        if memo1.contains(&encoded1) || memo2.contains(&encoded2) {
            return (PlayerId::One, 0);
        }
        memo1.insert(encoded1);
        memo2.insert(encoded2);
